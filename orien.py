...
st.title("オリエンテーリングAI (超高速キャッシュ版)")
uploaded_file = st.file_uploader("地図画像（PNG等）を選択してください", type=["png", "jpg", "jpeg"])

# ==========================================
# ★NEW: 激重な画像処理をキャッシュ化（1回だけ実行して結果を記憶する）
# ==========================================
@st.cache_data(show_spinner="AIが地図の色と地形を解析中... (※画像アップロード時やスライダー変更時のみ実行されます)")
...
if uploaded_file is not None:
    ...
    with st.sidebar.expander("🔍 AIの空間認識テスト"):
        ...
        st.image(ap_vis, caption="📍 アタックポイント候補", use_container_width=True)
